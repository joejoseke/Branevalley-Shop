...
const GadgetGuru: React.FC<GadgetGuruProps> = ({ products, onViewProduct }) => {
  ...
  return (
    <>
      <button
        onClick={toggleOpen}
        className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 bg-primary text-white p-3 sm:p-4 rounded-full shadow-lg hover:bg-secondary transition-transform transform hover:scale-110 z-50"
        aria-label="Open Gadget Guru Assistant"
      >
        <WandSparklesIcon className="w-7 h-7 sm:w-8 sm:h-8" />
      </button>

      {isOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center animate-fade-in p-4">
          <div className="bg-surface rounded-lg shadow-xl w-full max-w-2xl p-6 relative animate-slide-in">
            <button onClick={toggleOpen} className="absolute top-4 right-4 text-text-secondary hover:text-text-primary">
              <XIcon className="w-6 h-6" />
            </button>
            <h2 className="text-xl sm:text-2xl font-bold text-primary mb-2">Gadget Guru</h2>
            <p className="text-text-secondary mb-5">Describe what you need, and I'll find the perfect gadget for you!</p>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <input
                ...
                className="flex-grow bg-background border border-gray-600 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary w-full"
                onKeyDown={(e) => e.key === 'Enter' && handleRecommendation()}
              />
              <button onClick={handleRecommendation} disabled={isLoading} className="bg-secondary text-white font-bold py-2 px-4 rounded hover:bg-primary disabled:bg-gray-500 w-full sm:w-auto">
                {isLoading ? 'Thinking...' : 'Find'}
              </button>
            </div>
            {error && <p className="text-red-500 mt-2">{error}</p>}

            <div className="mt-6 h-72 sm:h-80 overflow-y-auto">
